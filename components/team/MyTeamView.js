...
function MyTeamView({ data }) {
  ...
  const [updateTeamLogo, { loading, error }] = useMutation(mutations.partialUpdateTeam, {
    onCompleted: (data) => {
  ...
  const handleTeamLogoUpdate = ({ path }) => {
    setLogoSrc(path);
    updateTeamLogo({ variables: { id: data.team._id, data: { logoSrc: path } } });
  };
