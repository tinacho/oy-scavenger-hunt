...
function MemberRm({ data, name }) {
    ...
    const onRemoveMember = () => {
        ...
        const newMembers = data.team.members.filter((member) => member.name !== name);
        ...
        removeMember({
            variables: {
                ...
                data: {
                    members: [
                        ...newMembers.map((member) => ({ name: member.name }))
                    ]
                },
            },
        });
    ...
    return (
        <Member onClick={onClick}>
            <Strong>{name}</Strong> {removeBtn && <button onClick={onRemoveMember}>X</button>}
        </Member>
    );
}
